...
-- first_n_ame como 'nombre Empleado' y 
-- last_name como 'Apellido Empleado' para los empleados con employee_number: 1002,1076,1088 y 1612.

SELECT employee_number AS NumeroEmpleado, last_name AS ApellidoEmpleado, first_name AS NameEmpleado
FROM employees
WHERE employee_number IN ('1002', '1076', '1088', '1612');

-- 5. Queremos ver ahora la 'ciudad' y 
-- los nombres de las empresas como 'nombre de la empresa ' de la tabla customers, 
-- que no estén en: Ireland, France, Germany.

SELECT city AS ciudad, customer_name AS NombreEmpresa
FROM customers
...
FROM customers
WHERE postal_code REGEXP '3' AND city REGEXP '^[^T]';
...
SELECT customer_name, address_line1, city, postal_code
FROM customers
WHERE postal_code REGEXP '[1234567890]';
